			  ...
			  char *comment)
{
	...
	c->version.flags = 0;
	c->comment = comment;
	c->major_parent = malloc(sizeof(struct commit));
	c->major_parent = NULL;
...
struct commit *add_minor_commit(struct commit *from, char *comment)
{
	...
	c = new_commit(from->version.major ,from->version.minor+1, comment);
...
struct commit *add_major_commit(struct commit *from, char *comment)
{
	...
	c = new_commit(from->version.major + 1, 0, comment);
	...
	if(from->version.minor == 0){
	  ...
	  parent = from->major_parent;
	  list_add(&c->major_list,&parent->major_list);
	}
